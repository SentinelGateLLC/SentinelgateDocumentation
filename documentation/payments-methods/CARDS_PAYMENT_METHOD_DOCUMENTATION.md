...
The customer is redirected to a payment page hosted by SentinelGate or the provider. For card payments, details are entered on their page. For MB Way, the customer provides their phone number to trigger a push notification to their banking app.
...
Customer → redirect to payment page → enter card(or phone in case of MB Way) → pay
